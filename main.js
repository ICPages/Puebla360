...
async function cargarNegocios() {
  ...
  // Obtener calificación real de Firebase para cada negocio
...
function createCard(n) {
  ...
  if (n.tarjtransf) iconos.push("💳");   // Agrega el ícono de tarjeta si es true
  if (n["24hrs"]) iconos.push("24 hrs"); // Agrega "24 hrs" si es true
  ...
  <div class="col-12 col-sm-6 col-lg-3">
    <a href="${n.pagina}?id=${n.id}" class="text-decoration-none text-dark">
      <div class="card business-card h-100 card-zoom">
        ...
        <div class="card-body">
          <h5 class="card-title fw-bold">${n.nombre}</h5>
...
function applyAllFilters() {
  ...
  if(searchText && searchText.trim()!==''){
    ...
    list = list.filter(n => (n.nombre + ' ' + n.descorta + ' ' + n.categoria).toLowerCase().includes(t));
  }
  ...
  if(activeFilters.sort === 'asc') {
    list.sort((a,b) => {
      if(a.estrellas == null && b.estrellas == null) return 0;
